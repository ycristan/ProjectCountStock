-- feat: merge final inclui TODO o inventário; item não contado por nenhuma equipe = 0.
-- Único delta vs 014: o loop externo percorre inventory_items (não só reconciliation_items).
-- Para um código sem contagem, o loop interno não soma nada → v_total=0 → linha 0/0 em combined_results.
-- ponytail: COALESCE(NULLIF(bpu,0),1) evita divisão por zero em itens com bpu 0/null (incompletos).

      ...
      <div className="space-y-4">
        {teams.map((team) => {
          ...
          return (
            <div
              key={team.id}
              className={`bg-white border rounded-xl overflow-hidden ${borderClass}`}
            >
              <div
                  ...
                  <div key={c.id} className="px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">{c.entry_count} itens lançados</div>
                      </div>
                    </div>
                    {c.finalized_at ? (
                      ...
                      <span className="text-xs text-slate-400">Em contagem...</span>
                    )}
                  </div>
                ))}
                {team.counters.length === 0 && (
                  <div className="px-4 py-3 text-xs text-slate-400">Nenhum contador criado.</div>
                )}
              </div>
            </div>
          )

...
// Snapshot storage/read boundary only; no signature/reconciliation UI claim.
sql(`update public.team_flows set phase='admin_review',revision=8 where team_id='${team}';
select private.build_team_result_snapshot('${team}',8,'${people[2].member}','[]'::jsonb);`)
assert.equal(checked(await monitor.from('team_result_versions').select('id').eq('team_id',team)).length,1)
assert.deepEqual(checked(await people[0].client.from('team_result_versions').select('id').eq('team_id',team)),[])
assert.equal(checked(await people[3].client.from('team_result_versions').select('id').eq('team_id',team)).length,1)
denied(await anonymous.from('team_result_versions').select('id'))
denied(await privileged.from('team_result_versions').update({team_name:'tampered'}).eq('team_id',team))
console.log('PASS: real HTTP snapshot reads preserve counter blindness; anonymous and direct writes denied')
